"""
Rolling-origin XGBoost training
...
def main() -> None:
    ...
    for max_depth in [6, 10]:
        for learning_rate in [0.05, 0.1]:
            param_grid.append({"max_depth": max_depth, "learning_rate": learning_rate})
    ...
    for h in HORIZONS:
        ...
        # Hyperparameter search across folds
        best_param = None
        best_score = None
        for p in param_grid:
            fold_scores = []
        ...
        for fold in val_folds:
            ...
            yhat_train_open = np.expm1(pipe_cv.predict(X_train))
        ...
        model_path = os.path.join(OUT_ARTIFACTS_DIR, f"xgboost_h{h}.joblib")
        joblib.dump(pipe_final, model_path)
        params_path = os.path.join(OUT_ARTIFACTS_DIR, f"xgboost_h{h}_params.json")
