...
def get_run_configs() -> List[Dict[str, Any]]:
    long_cfg = {
        ...
        "metrics_filename": "flaml_metrics.csv",
        "artifact_prefix": "flaml",
    }
    quick_cfg = {
        "tag": "quick",
        "time_budget": QUICK_TIME_BUDGET_PER_RUN,
        "metrics_filename": "flaml_metrics_quick.csv",
        "artifact_prefix": "flaml_quick",
    }
...
def train_and_evaluate_final(
    ...
    model_path = os.path.join(OUT_ARTIFACTS_DIR, f"{artifact_prefix}_h{h}.joblib")
    ...
    for cfg in get_run_configs():
        ...
        for h in HORIZONS:
            ...
            for rec in final_records:
                results.append(rec)
            print(f"Saved model: {os.path.join(OUT_ARTIFACTS_DIR, f'{artifact_prefix}_h{h}.joblib')}")
